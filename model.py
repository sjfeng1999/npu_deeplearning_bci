...
class ConvLSTM_init(nn.Module):
    ...
    def __init__(
        ...
        self.peace_sequence_size = peace_sequence_lens
        self.window_size = sequence_lens // time_lens
        self.device = torch.device('cuda')

        self.input_size, self.emd_size = self._adaptive_feature_size()
        self.subconv = SubConvNet(in_channel=in_channel, out_channel=4)
        self.emdnet = SubConvNet(in_channel=in_channel, out_channel=1)
        self.emdlinear = nn.Linear(self.emd_size, self.hidden_size)
    ...
    def _adaptive_feature_size(self):
        ...
        px = torch.zeros(1, self.in_channel, self.peace_sequence_size)
...
class FrequencyAttentionNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.stack(
            [self.band_net[i](x[i].reshape(batch_size * self.per_channel, 1, -1)) for i in range(self.band_size)], 1)
        x = x.view(batch_size, self.in_channel, -1)
